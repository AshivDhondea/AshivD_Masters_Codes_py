## Pj016_CDEKFpaper_Code002.py
# Based on Pj014_Simulating_TwoBody_save_data_radar.py
# ------------------------- #
# Description:
...
dt = 0.001;#[s] changed 29.7.16
t_end = 300;#[s] Target observed only for 2 seconds. changed 29.7.16
...
index = 0;
with open('Pj016_CDEKFpaper_Code001.txt') as fp:
    for line in fp:
        data = line.split();
...
Tk = 8; # [s]
t_y = np.arange(0,t_end,Tk,dtype=np.float64); # time vector for measurements.
...
for index in range(0,len(t_y)): 
    ...
    pos = np.array([xtrue[0,index*(Tk)],xtrue[1,index*(Tk)],xtrue[2,index*(Tk)]],dtype=np.float64);
    y_meas[:,index] = Obs.fnH(pos)+ vn;
    xradar[:,index] = xtrue[:,index*(Tk)];
    print index*(Tk);
...
fname = 'Pj016_CDEKFpaper_Code002_y.txt';
...
fname = 'Pj016_CDEKFpaper_Code002_x.txt';
f = open(fname, 'w') # Create data file;
for index in range (0,len(t_y)):
